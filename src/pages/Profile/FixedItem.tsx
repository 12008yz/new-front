...
import { usePutFixDescriptionMutation } from "../../app/services/users/UserServicer";
...
const FixItem: React.FC<IfixedItem> = ({ fixedItem, isSameUser, setRefresh }) => {
    ...
    const [putFixDescription] = usePutFixDescriptionMutation();
    console.log("Fixed Item Data:", fixedItem);
    const updateFixDescription = async (description: string) => {
        try {
            const res = await putFixDescription(description).unwrap();
            if (res.success) {
    ...
    return (
        <div
            ...
            style={{
                backgroundImage: `linear-gradient(270deg, ${Rarities.find((rarity) => rarity.id == fixedItem.rarity)?.color} 20%, rgba(0,0,0,0) 100%)`,
            }}
        >
